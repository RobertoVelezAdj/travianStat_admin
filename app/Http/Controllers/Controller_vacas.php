...
class Controller_vacas extends Controller
{
    ...
    public function actualizar_pago2(request $info)
    {
        $idUsu =auth()->id() ;
        ...
        for($i = 0; $i < sizeof($cadena);$i=$i+1)
        {
            ...
            if($contador==$nombre&& strlen($cadena[$i])>0){
                ...
                $distancia_Vaca =$cadena[$i];
            }else if($contador==$result&& strlen($cadena[$i])>0)
            {
                $query = "select a.id as id_aldea, ina.idAldea as aldeaVaca, ina.coord_x, ina.coord_y, ina.poblacion, ina.NombreAldea, s.id as idServer from aldea_inac ina, servidor s, users u, aldea a where a.id_usuario = u.id and ina.NombreAldea = '".trim($nombrealdea)."' and calcular_distancia(a.coord_x,a.coord_y,ina.coord_x,ina.coord_y) = ".$distancia_Vaca." and u.servidor = s.id and s.fch_mod = ina.created_at and a.id  =".$info->id_aldea;
                $resultado= DB::select($query);
                foreach ($resultado as $a){
    ...
    public function eliminarVaca(request $info)
    {
        $idUsu =auth()->id() ;
